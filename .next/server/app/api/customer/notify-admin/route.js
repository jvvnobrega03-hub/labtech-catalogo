(()=>{var a={};a.id=608,a.ids=[608],a.modules={261:a=>{"use strict";a.exports=require("next/dist/shared/lib/router/utils/app-paths")},3295:a=>{"use strict";a.exports=require("next/dist/server/app-render/after-task-async-storage.external.js")},8128:a=>{"use strict";a.exports=require("next/dist/server/runtime-reacts.external.js")},10846:a=>{"use strict";a.exports=require("next/dist/compiled/next-server/app-page.runtime.prod.js")},19121:a=>{"use strict";a.exports=require("next/dist/server/app-render/action-async-storage.external.js")},27910:a=>{"use strict";a.exports=require("stream")},28354:a=>{"use strict";a.exports=require("util")},29294:a=>{"use strict";a.exports=require("next/dist/server/app-render/work-async-storage.external.js")},44870:a=>{"use strict";a.exports=require("next/dist/compiled/next-server/app-route.runtime.prod.js")},55511:a=>{"use strict";a.exports=require("crypto")},55958:(a,b,c)=>{"use strict";c.r(b),c.d(b,{POST:()=>g,PUT:()=>h});var d=c(23211),e=c(43151),f=c(68681);async function g(a){try{let b="https://labtechcatalogo.supabase.co",g=process.env.SUPABASE_SERVICE_ROLE_KEY;if(!b||!g)return d.NextResponse.json({success:!1,error:"Configura\xe7\xe3o do servidor incompleta"},{status:500});let h=(0,e.UU)(b,g),{customerId:i}=await a.json();if(!i)return d.NextResponse.json({success:!1,error:"ID do cliente n\xe3o fornecido"},{status:400});let{data:j,error:k}=await h.from("customer_profiles").select("*").eq("id",i).single();if(k||!j)return d.NextResponse.json({success:!1,error:"Cliente n\xe3o encontrado"},{status:404});if(j.approval_notification_sent_at)return d.NextResponse.json({success:!0,message:"Notifica\xe7\xe3o j\xe1 enviada anteriormente"});let l=c(55511),m=()=>l.randomBytes(32).toString("hex"),n=m(),o=m(),p=l.createHash("sha256").update(n).digest("hex"),q=l.createHash("sha256").update(o).digest("hex"),{error:r}=await h.from("approval_tokens").insert([{customer_id:j.id,token_hash:p,action:"APPROVE",expires_at:new Date(Date.now()+6048e5).toISOString()},{customer_id:j.id,token_hash:q,action:"REJECT",expires_at:new Date(Date.now()+6048e5).toISOString()}]);if(r)return console.error("Erro ao criar tokens:",r),d.NextResponse.json({success:!1,error:"Erro ao criar tokens de aprova\xe7\xe3o"},{status:500});let s=await (0,f.Mj)(j,n);return s.success?await h.from("customer_profiles").update({approval_notification_sent_at:new Date().toISOString(),approval_notification_attempts:(j.approval_notification_attempts||0)+1}).eq("id",j.id):await h.from("customer_profiles").update({approval_notification_error:s.error,approval_notification_attempts:(j.approval_notification_attempts||0)+1}).eq("id",j.id),d.NextResponse.json({success:s.success,message:s.success?"Notifica\xe7\xe3o enviada com sucesso":"Erro ao enviar notifica\xe7\xe3o",error:s.error})}catch(a){return console.error("Erro ao processar notifica\xe7\xe3o:",a),d.NextResponse.json({success:!1,error:"Erro interno do servidor"},{status:500})}}async function h(a){try{let b="https://labtechcatalogo.supabase.co",g=process.env.SUPABASE_SERVICE_ROLE_KEY;if(!b||!g)return d.NextResponse.json({success:!1,error:"Configura\xe7\xe3o do servidor incompleta"},{status:500});let h=(0,e.UU)(b,g),{customerId:i}=await a.json();if(!i)return d.NextResponse.json({success:!1,error:"ID do cliente n\xe3o fornecido"},{status:400});let{data:j,error:k}=await h.from("customer_profiles").select("*").eq("id",i).single();if(k||!j)return d.NextResponse.json({success:!1,error:"Cliente n\xe3o encontrado"},{status:404});if("PENDING"!==j.status)return d.NextResponse.json({success:!1,error:"Cliente n\xe3o est\xe1 pendente"},{status:400});let{data:l}=await h.from("approval_tokens").select("id").eq("customer_id",i).eq("used_at",null).gt("expires_at",new Date().toISOString()),m=c(55511),n=m.randomBytes(32).toString("hex"),o=m.createHash("sha256").update(n).digest("hex");await h.from("approval_tokens").insert({customer_id:j.id,token_hash:o,action:"APPROVE",expires_at:new Date(Date.now()+6048e5).toISOString()});let p=await (0,f.Mj)(j,n);return await h.from("customer_profiles").update({approval_notification_sent_at:p.success?new Date().toISOString():null,approval_notification_error:p.error||null,approval_notification_attempts:(j.approval_notification_attempts||0)+1}).eq("id",j.id),d.NextResponse.json({success:p.success,message:p.success?"Notifica\xe7\xe3o reenviada com sucesso":"Erro ao enviar notifica\xe7\xe3o",error:p.error})}catch(a){return console.error("Erro ao reenviar notifica\xe7\xe3o:",a),d.NextResponse.json({success:!1,error:"Erro interno do servidor"},{status:500})}}},57075:a=>{"use strict";a.exports=require("node:stream")},63033:a=>{"use strict";a.exports=require("next/dist/server/app-render/work-unit-async-storage.external.js")},68681:(a,b,c)=>{"use strict";c.d(b,{IE:()=>j,Mj:()=>h,_w:()=>i});var d=c(77483);let e=process.env.ADMIN_APPROVAL_EMAIL||"",f=process.env.EMAIL_FROM||"LABTECH <naoresponda@labtech.com.br>",g="http://localhost:3000";async function h(a,b){if(!e)return console.error("ADMIN_APPROVAL_EMAIL n\xe3o configurado"),{success:!1,error:"E-mail do respons\xe1vel n\xe3o configurado"};let c=new d.u(process.env.RESEND_API_KEY),h=`${g}/aprovacao?token=${b}&action=approve`,i=`${g}/aprovacao?token=${b}&action=reject`,j="CNPJ"===a.document_type?`${a.document.slice(0,2)}.${a.document.slice(2,5)}.${a.document.slice(5,8)}/${a.document.slice(8,12)}-${a.document.slice(12)}`:`${a.document.slice(0,3)}.${a.document.slice(3,6)}.${a.document.slice(6,9)}-${a.document.slice(9)}`,k=`(${a.phone.slice(0,2)}) ${a.phone.slice(2,7)}-${a.phone.slice(7)}`,l=`
<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
